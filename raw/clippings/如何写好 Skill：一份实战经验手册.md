---
title: "如何写好 Skill：一份实战经验手册"
source: "https://zhuanlan.zhihu.com/p/2045872710375428908"
author:
  - "[[腾讯技术工程​编程话题下的优秀答主]]"
published:
created: 2026-06-10
description: "作者：jackjchou 这篇文章把我们写 Skill 踩过的坑、总结出的经验，再加上 Anthropic 官方的一些好做法，整理到了一起。希望能帮你少走弯路，把团队积累的知识真正\"喂\"给 AI，让它干活更靠谱。 本文示例以…"
tags:
  - "clippings"
---
76 人赞同了该文章
...
> 这篇文章把我们写 Skill 踩过的坑、总结出的经验，再加上 [Anthropic](https://zhida.zhihu.com/search?content_id=276214938&content_type=Article&match_order=1&q=Anthropic&zhida_source=entity) 官方的一些好做法，整理到了一起。希望能帮你少走弯路，把团队积累的知识真正"喂"给 AI，让它干活更靠谱。 本文示例以 [Go 语言](https://zhida.zhihu.com/search?content_id=276214938&content_type=Article&match_order=1&q=Go+%E8%AF%AD%E8%A8%80&zhida_source=entity) 为主，兼顾 Python、Java 等语言，所有原则和技巧适用于任何编程语言。
...
| 想了解 [MCP](https://zhida.zhihu.com/search?content_id=276214938&content_type=Article&match_order=1&q=MCP&zhida_source=entity) 和外部服务集成 | 六 |
| Skill 跑不通，想排查问题 | 九 → 十 |
...
说白了，Skill 就是给 AI 编程助手（ [Claude Code](https://zhida.zhihu.com/search?content_id=276214938&content_type=Article&match_order=1&q=Claude+Code&zhida_source=entity) 、CodeBuddy 等）"加装"的能力包。本质上，它是一种 **结构化的 [Prompt Engineering](https://zhida.zhihu.com/search?content_id=276214938&content_type=Article&match_order=1&q=Prompt+Engineering&zhida_source=entity)** ——通过标准的文件格式，把分散在人脑中的领域知识、操作流程和最佳实践，转化为 AI 可理解、可执行的指令集。
...
- 在线工具： [OpenAI Tokenizer](https://link.zhihu.com/?target=https%3A//platform.openai.com/tokenizer)

> ⚠️ 以上数据基于 Claude tokenizer 估算，不同模型（GPT-4、 [Gemini](https://zhida.zhihu.com/search?content_id=276214938&content_type=Article&match_order=1&q=Gemini&zhida_source=entity) 等）的 tokenizer 实现不同，实际消耗会有 ±20% 的差异。
...
> 💡 **开始写之前，先想清楚一件事** ：你的 Skill 是一件事还是好几件事？如果内容预计会超过 500 行，或者包含多个独立的工作流程，建议先看" [Skill 太长了怎么办——拆（模块化）](https://link.zhihu.com/?target=https%3A//km.woa.com/articles/show/654663%23%25E5%259B%259Bskill-%25E5%25A4%25AA%25E9%2595%25BF%25E4%25BA%2586%25E6%2580%258E%25E4%25B9%2588%25E5%258A%259E%25E6%258B%2586%25E6%25A8%25A1%25E5%259D%2597%25E5%258C%2596) "，想好结构再动笔。磨刀不误砍柴工。
...
| 情形 | 特征 | 处理方案 | 自动/手动 |
| --- | --- | --- | --- |
| 直接 import | import " [github.com/old/pkg](https://link.zhihu.com/?target=http%3A//github.com/old/pkg) " | 直接替换 import 路径 | 自动 |
| 别名 import | import alias " [github.com/old/pkg](https://link.zhihu.com/?target=http%3A//github.com/old/pkg) " | 替换路径，保留别名 | 自动 |
| 点导入 | . " [github.com/old/pkg](https://link.zhihu.com/?target=http%3A//github.com/old/pkg) " | 替换路径，检查冲突符号 | 自动 |
...
| 字段 | 类型 | 必填 | 说明 | 示例 |
| --- | --- | --- | --- | --- |
| module | string | 是 | Go 模块路径 | " [github.com/example/my-p](https://link.zhihu.com/?target=http%3A//github.com/example/my-project) " |
| go | string | 是 | Go 最低版本要求 | "1.21" |
| require | []dependency | 否 | 依赖模块列表 | [github.com/gin-gonic/gi](https://link.zhihu.com/?target=http%3A//github.com/gin-gonic/gin) v1.9.1 |
...
| 忽略 MCP 生态已有的 Server | 先查 MCP Server 列表 再决定自建 |
...
| 手动安装 | git clone [github.com/anthropics/s](https://link.zhihu.com/?target=https%3A//github.com/anthropics/skills.git) 后复制 skills/skill-creator 到 ~/.codebuddy/skills/ |
...
# 1. 检查是否残留旧版引用
echo "=== 检查旧版引用 ==="
grep -rn "old-http-client" . --include="*.go" && echo "❌ 仍有旧版引用" || echo "✅ 旧版引用已清理"
...
- **避免了常见反模式** ：对照" [常见反模式](https://link.zhihu.com/?target=https%3A//km.woa.com/articles/show/654663%23%25E5%258D%2581%25E4%25BA%258C%25E5%2588%25AB%25E8%25B8%25A9%25E8%25BF%2599%25E4%25BA%259B%25E5%259D%2591%25E5%25B8%25B8%25E8%25A7%2581%25E5%258F%258D%25E6%25A8%25A1%25E5%25BC%258F) "逐条检查
...
| 资源 | 说明 | 链接 |
| --- | --- | --- |
| Anthropic 官方 Skills 仓库 | 包含官方示例和 Skill Creator 元技能 | [github.com/anthropics/s](https://link.zhihu.com/?target=http%3A//github.com/anthropics/skills) |
| Skill Creator | Anthropic 官方的"创建 Skill 的 Skill" | skill-creator/SKILL.md |
| OpenSkills 生态 | 通用的 Skills 加载/管理工具 | [github.com/openskills](https://link.zhihu.com/?target=http%3A//github.com/openskills) |
| MCP Server 列表 | 已有的 MCP Server 生态 | [github.com/modelcontext](https://link.zhihu.com/?target=http%3A//github.com/modelcontextprotocol/servers) |
| OpenAI Tokenizer | 在线 Token 计数工具 | [platform.openai.com/tok](https://link.zhihu.com/?target=http%3A//platform.openai.com/tokenizer) |

---

写 Skill 这件事，说到底就是把你脑子里"知道怎么做"的经验，变成 AI 也能"照着做"的格式。第一版肯定不完美，但没关系，用着用着就知道哪里需要改了。好的 Skill 都是在实际使用中一点点打磨出来的。

发布于 2026-06-05 18:15・广东
